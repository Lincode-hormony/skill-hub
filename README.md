...
- **批量打标签**：选择多个 Skills 一次性添加相同标签，便于按系列分组
- **智能标签识别**：安装时自动识别仓库来源并建议标签（如 `obra/superpowers` → `superpowers`）
- **标签筛选**：在 Skills 列表中通过标签快速筛选
- **多标签选择**：支持同时选择多个标签进行精确筛选
- **标签统计**：查看每个标签下有多少个 Skill
- **追加模式**：新标签追加到现有标签，不会覆盖已有分类
...
- **标签快速选择**：在项目管理中通过标签快速批量勾选 Skills
- **按标签分组显示**：Skills 按标签分组展示，一目了然
- **OR 逻辑**：选择多个标签会累积勾选相关 Skills
- **未分类支持**：没有标签的 Skills 也能正常选择
